...
//       platform: [],
//       keywords: '',
//       urls: ''  // Changed from array to single string
//     },
//   ]);
...
//       const newSources = [...sources];
//       newSources[index].keywords = (newSources[index].keywords ? newSources[index].keywords + ', ' : '') + newSources[index].keywordInput.trim();
...
//       const newSources = [...sources];
//       newSources[index].urls = (newSources[index].urls ? newSources[index].urls + ', ' : '') + newSources[index].urlInput.trim();
...
//   const handleDeleteKeyword = (sourceIndex) => {
//     const newSources = [...sources];
//     newSources[sourceIndex].keywords = '';
//     setSources(newSources);
//   };

//   const handleDeleteUrl = (sourceIndex) => {
//     const newSources = [...sources];
//     newSources[sourceIndex].urls = '';
//     setSources(newSources);
//   };
...
//             urls: source.urls,  // Single string
...
//         setFilterName('');
//         setDescription('');
//         setSources([{ source: '', platform: [], keywords: '', urls: '' }]);  // Reset sources
...
//                     />
//                     <div className="mt-2">
//                       {source.keywords.split(',').map((keyword, keyIndex) => (
//                         <Badge
//                           key={keyIndex}
//                           pill
//                           bg="dark"
...
//                             onClick={() => handleDeleteKeyword(sourceIndex)}
//                           >
//                             ×
//                           </Button>
//                         </Badge>
...
//                     />
//                     <div className="mt-2">
//                       {source.urls.split(',').map((url, urlIndex) => (
//                         <Badge
//                           key={urlIndex}
//                           pill
//                           bg="dark"
...
//                             variant="link"
//                             className="text-light p-0 ms-2"
//                             onClick={() => handleDeleteUrl(sourceIndex)}
//                           >
//                             ×
//                           </Button>
//                         </Badge>
...
const AddNewFilter = () => {
  ...
  const handleKeywordKeyDown = (index, event) => {
    if (event.key === 'Enter' && sources[index].keywordInput.trim()) {
      const newSources = [...sources];
      newSources[index].keywords = [newSources[index].keywords[0] + (newSources[index].keywords[0] ? ', ' : '') + newSources[index].keywordInput.trim()];
      newSources[index].keywordInput = '';
      setSources(newSources);
      event.preventDefault();
    }
  ...
  const handleUrlKeyDown = (index, event) => {
    if (event.key === 'Enter' && sources[index].urlInput.trim()) {
      const newSources = [...sources];
      newSources[index].urls = [newSources[index].urls[0] + (newSources[index].urls[0] ? ', ' : '') + newSources[index].urlInput.trim()];
      newSources[index].urlInput = '';
      setSources(newSources);
      event.preventDefault();
    }
  ...
  const handleSubmit = async () => {
    const postData = {
      ...
      filter_criteria: sources.map((source) => {
        if (source.source === 'rss feed') {
          return {
            source: source.source,
            urls: source.urls,  // Single string in an array
            keywords: source.keywords
          };
        } else {
          return {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("startas", startPayload)
        await axios.post(
          'http://5.180.148.40:9006/api/osint-man/v1/start',
          startPayload,
          { headers: { Authorization: `Bearer ${token}` } }
